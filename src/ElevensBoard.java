...
public class ElevensBoard extends Board {
	...
	private List<Integer> findPairSum11(List<Integer> selectedCards) {
		...
		List<Integer> goldenBoys = new ArrayList<Integer>();
		for (int sk1 = 0; sk1 < selectedCards.size(); sk1++) {
			int k1 = selectedCards.get(sk1).intValue();
			for (int sk2 = sk1 + 1; sk2 < selectedCards.size(); sk2++) {
				int k2 = selectedCards.get(sk2).intValue();
				if (cardAt(k1).pointValue() + cardAt(k2).pointValue() == 11) {
					goldenBoys.add(new Integer(k1));
					goldenBoys.add(new Integer(k2));
					return goldenBoys;
				}
			}
		}
	...
	private boolean playPairSum11IfPossible() {
		List<Integer> locOfPlay = findPairSum11(cardIndexes());
		if (locOfPlay.size() == 2) {
			replaceSelectedCards(locOfPlay);
			return true;
		}
		return false;
	}
	...
	private boolean playJQKIfPossible() {
		List<Integer> locOfPlay = findJQK(cardIndexes());
		if (locOfPlay.size() == 3) {
			replaceSelectedCards(locOfPlay);
			return true;
		}
